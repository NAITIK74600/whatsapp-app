...
### Added
- **Hostinger Web App support**: New plain-JS entry `server.mjs` (syncs DB schema, bootstraps the admin from `ADMIN_EMAIL`/`ADMIN_PASSWORD`, then starts the TS server via `tsx`). Managed Node hosts require a `.js/.mjs` entry file. See `docs/HOSTINGER_DEPLOYMENT.md`.
- **Keep-alive self ping** (`KEEP_ALIVE_*` env vars) and public `GET /api/health` endpoint so hosts that stop idle processes (Hostinger) don't drop WhatsApp connections.
- **AI auto-reply integration**: Optional OpenAI-compatible chat completions support with `AI_*` env vars and per-session Bot Settings controls.
- **Hostinger production env template**: Added `hostinger-production.env.example` with literal production URL placeholders for hPanel import.
- `npm run typecheck` and `npm run lint:fix` scripts; `engines.node >= 20` declared.

### Changed
- `npm run build` now runs `prisma generate` first; `npm start` runs `node server.mjs` (PM2 ecosystem and Dockerfile use the same entry).
- `tsx` and `prisma` moved to `dependencies` (needed at runtime by the entry file).
- ESLint: `no-explicit-any`, `ban-ts-comment`, `no-unescaped-entities`, `set-state-in-effect` downgraded to warnings; `require()` allowed in `scripts/`. Lint now passes with 0 errors.
- Auto-reply now supports the `STARTS_WITH` match type that the dashboard already exposed.
- `/icon` is rendered dynamically so branding changes apply without a rebuild.

### Fixed
- Unhandled promise rejection when `waManager.loadSessions()` failed at startup (could crash the process on Node 20+).
- Build no longer opens database connections during `next build` (layout metadata / favicon), which failed in build sandboxes without DB access.
- `.dockerignore` excluded `*.patch`, so the Baileys patch was never applied in Docker images; Dockerfile also now ships `next.config.mjs` to the runtime image.
- Server binds `0.0.0.0` explicitly; `HOSTNAME` (machine name on Linux hosts) is display-only.
- Removed stale scratch files (`ts_errors*.log`, `eslint-errors.txt`, `test-*.ts/js`, …).
