...
- **[Hostinger Deployment Guide](docs/HOSTINGER_DEPLOYMENT.md)**: Step-by-step deploy on Hostinger Web App (Node.js) hosting + Hostinger MySQL. Use [hostinger-production.env.example](hostinger-production.env.example) for hPanel environment variables.
...
> [!TIP]
> **Deploying on Hostinger shared/cloud hosting?** Follow the dedicated **[Hostinger Deployment Guide](docs/HOSTINGER_DEPLOYMENT.md)** — it uses the hPanel *Node.js web app* feature with the **Other** preset, `server.mjs` as the entry file, and [hostinger-production.env.example](hostinger-production.env.example) for production URL/database variables.
...
1. **Build the application**:
   ...
   npm run build
   ```
   (Runs `prisma generate` then `next build`. To run without PM2: `npm start` — this syncs the DB schema, creates the admin from `ADMIN_EMAIL`/`ADMIN_PASSWORD` if needed, and starts the server.)
