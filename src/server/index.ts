...
app.prepare().then(() => {
  ...
  server.listen(port, "0.0.0.0", () => {
    ...
    // Managed hosts (e.g. Hostinger Web Apps) stop the process after a period without incoming traffic,
    // which would drop every WhatsApp connection. Hitting our own public URL counts as traffic.
    // ponytail: only works while the process is alive — pair with an external uptime pinger for cold starts.
    const KEEP_ALIVE_URL = process.env.KEEP_ALIVE_URL || (process.env.BASE_URL && !/localhost|127\.0\.0\.1/.test(process.env.BASE_URL) ? `${process.env.BASE_URL.replace(/\/$/, "")}/api/health` : "");
    if (KEEP_ALIVE_URL && process.env.KEEP_ALIVE_ENABLED !== "false") {
      const minutes = parseInt(process.env.KEEP_ALIVE_INTERVAL_MINUTES || "4", 10);
      setInterval(() => fetch(KEEP_ALIVE_URL).catch(() => {}), minutes * 60 * 1000);
      logger.info("Server", `Keep-alive ping every ${minutes}m -> ${KEEP_ALIVE_URL}`);
    }
    // ----------------------------
